...
class PhysicalXPassTests(unittest.TestCase):
    ...
    def test_pc_xpass_raw_control_and_conditional_normalization(self) -> None:
        raw = pc_xpass_raw_control(np.asarray([-0.3, 0.0, 0.5], dtype=float))

        self.assertAlmostEqual(float(raw[0]), 0.5)
        self.assertGreater(float(raw[1]), 0.98)
        self.assertGreater(float(raw[2]), 0.999)
        custom = pc_xpass_raw_control_with_params(np.asarray([-0.2, 0.0], dtype=float), power=20.0, inflection_point=0.2)
    ...
    def test_pc_xpass_endpoint_proportional_normalization(self) -> None:
        under_one = np.asarray([0.2, 0.3, 0.4], dtype=float)
    ...
    def test_pc_xpass_endpoint_share_uses_receiver_raw_times_cumulative_share(self) -> None:
        cases = [
            (np.asarray([0.5, 0.5]), 0.25),
            (np.asarray([1.0, 1.0]), 0.5),
            (np.asarray([0.8, 0.4]), 0.8 * 0.8 / 1.2),
            (np.asarray([0.8, 0.0]), 0.8),
            (np.asarray([0.0, 0.0]), 0.0),
            (np.asarray([0.6, 0.2, 0.1]), 0.6 * 0.6 / 0.9),
        ]

        for raw, expected in cases:
            output = pc_xpass_endpoint_control_probabilities(
                raw,
                receiver_index=0,
        ...
        default_output = pc_xpass_endpoint_control_probabilities(
            np.asarray([0.5, 0.5]),
            receiver_index=0,
        )
        self.assertAlmostEqual(float(default_output[0]), 0.25)

    def test_pc_xpass_endpoint_share_preserves_nonfinite_receiver_and_applies_defender_boost(self) -> None:
        nonfinite = pc_xpass_endpoint_control_probabilities(
            np.asarray([np.nan, 0.4]),
            receiver_index=0,
            endpoint_normalization=PC_XPASS_ENDPOINT_NORMALIZATION_SHARE,
        )
        self.assertTrue(np.isnan(nonfinite[0]))

        boosted = pc_xpass_endpoint_control_probabilities(
            np.asarray([0.8, 0.4, 0.2]),
            receiver_index=0,
            player_teams=np.asarray(["attack", "defense", "attack"]),
            endpoint_normalization=PC_XPASS_ENDPOINT_NORMALIZATION_SHARE,
            boost_def_endpoint_control=2.0,
        )
        self.assertAlmostEqual(float(boosted[0]), 0.8 * 0.8 / (0.8 + 0.8 + 0.2))
    ...
    def test_pc_xpass_endpoint_normalization_modes_for_receiver(self) -> None:
        ...
        share = pc_xpass_endpoint_control_probabilities(
            raw,
            receiver_index=0,
            endpoint_normalization=PC_XPASS_ENDPOINT_NORMALIZATION_SHARE,
        )
        ...
        self.assertAlmostEqual(float(share[0]), 1 / 3)
    ...
    def test_compute_graph_pc_xpass_uses_split_lane_and_control_sigmoids(self) -> None:
        ...
        def fake_raw_control(margins: np.ndarray, *, power: float, inflection_point: float) -> np.ndarray:
            calls.append((float(power), float(inflection_point)))
    ...
    def test_pc_xpass_motion_and_grid_parameters_affect_helpers(self) -> None:
        ...
        target_y = np.asarray([0.0], dtype=float)
        t_ball = np.asarray([1.0], dtype=float)

        fast_margin = _pc_xpass_arrival_margins(player_pos, target_x, target_y, t_ball, reaction_time=0.25, max_player_speed=10.0)
        slow_margin = _pc_xpass_arrival_margins(player_pos, target_x, target_y, t_ball, reaction_time=0.25, max_player_speed=5.0)
        long_reaction_margin = _pc_xpass_arrival_margins(player_pos, target_x, target_y, t_ball, reaction_time=0.5, max_player_speed=10.0)
        split_margin = _pc_xpass_arrival_margins(
        ...
        self.assertAlmostEqual(float(fine_grid[1] - fine_grid[0]), 1.5)
    ...
    def test_compute_graph_pc_xpass_uses_custom_min_speed_and_radial_gridsize(self) -> None:
        ...
        self.assertEqual(float(row["home_2__speed"]), 5.0)
        distance = float(row["home_2__distance"])
        self.assertAlmostEqual(distance % 2.0, 0.0)
    ...
    def test_compute_graph_pc_xpass_splits_lane_and_control_teammate_ignoring(self) -> None:
        ...
        self.assertEqual(
            float(lane_ignores_teammate["home_2__lane_survival"]),
            float(lane_considers_teammate["home_2__lane_survival"]),
        )
        self.assertEqual(float(lane_ignores_teammate["home_2"]), float(lane_considers_teammate["home_2"]))
    ...
    def test_pc_xpass_metadata_records_split_teammate_policies(self) -> None:
        metadata = pc_xpass_metadata(
            ...
            max_player_speed_off=7.0,
            max_player_speed_def=4.0,
            min_speed=2.0,
            radial_gridsize=1.5,
            lane_power=12.0,
            lane_inflection_point=0.4,
        ...
        self.assertEqual(metadata["lane_function"], "sigmoid")
        self.assertEqual(metadata["lane_power"], 12.0)
        self.assertEqual(metadata["lane_inflection_point"], 0.4)
    ...
    def test_pc_xpass_metadata_defaults_split_speeds_to_overall_speed(self) -> None:
        ...
        self.assertEqual(metadata["max_player_speed"], 6.0)
        self.assertEqual(metadata["max_player_speed_off"], 6.0)
        self.assertEqual(metadata["max_player_speed_def"], 6.0)
    ...
    def test_pc_xpass_cache_rejects_old_shared_control_function_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir) / "pc_xpass"
            cache_dir.mkdir(parents=True, exist_ok=True)
            metadata = pc_xpass_metadata(PHYSICAL_XPASS_TEAMMATE_POLICY_CONSIDER)
            metadata["control_function_power"] = 20.0
            metadata["control_function_inflection_point"] = 0.2
            metadata["control_function_gamma"] = 1.0
            for key in [
                "lane_function",
                "lane_power",
            ...
            with self.assertRaisesRegex(ValueError, "lane_function"):
                prewarm_physical_xpass_runtime_cache(
                    [],
                    cache_dir=cache_dir,
                    source=PC_XPASS_SOURCE,
    ...
    def test_pc_xpass_cache_rejects_old_metadata_missing_split_speed_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_dir = Path(tmpdir) / "pc_xpass"
            cache_dir.mkdir(parents=True, exist_ok=True)
            metadata = pc_xpass_metadata(PHYSICAL_XPASS_TEAMMATE_POLICY_CONSIDER)
            metadata.pop("max_player_speed_off", None)
            metadata.pop("max_player_speed_def", None)
            (cache_dir / "metadata.json").write_text(json.dumps(metadata), encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "max_player_speed_off"):
                prewarm_physical_xpass_runtime_cache(
                    [],
                    cache_dir=cache_dir,
                    source=PC_XPASS_SOURCE,
    ...
    def test_runtime_visualization_xpass_loader_reads_dynamic_pc_top_n_metric(self) -> None:
                ...
                [
                    {
                        "match_id": "match_1",
                        "action_index": 3,
                        "home_1__top50_xpass": 0.31,
                        "home_2__top50_xpass": 0.73,
                    }
                ]
            ).to_parquet(matches_dir / "match_1.parquet", index=False)

            top50 = load_runtime_physical_xpass_visualization_component(
                cache_dir,
                "match_1",
                3,
                metric="top50_xpass",
            )
            direct_top50 = load_physical_xpass_component(
                cache_dir,
                "match_1",
                3,
                metric="top50_xpass",
        ...
        self.assertEqual(top50.name, 3)
        self.assertEqual(top50.index.tolist(), ["home_1", "home_2"])
        self.assertAlmostEqual(float(top50["home_2"]), 0.73)
        self.assertEqual(direct_top50.name, "top50_xpass")
        self.assertAlmostEqual(float(direct_top50["home_1"]), 0.31)
    ...
    def test_generate_physical_xpass_cli_pc_xpass_uses_pc_defaults(self) -> None:
        ...
        self.assertEqual(args.min_speed, PC_XPASS_DEFAULT_MIN_SPEED)
        self.assertEqual(args.radial_gridsize, PC_XPASS_DEFAULT_RADIAL_GRIDSIZE)
        self.assertEqual(args.lane_power, PC_XPASS_DEFAULT_LANE_POWER)
        ...
        self.assertEqual(args.endpoint_normalization, PC_XPASS_ENDPOINT_NORMALIZATION_SHARE)
        ...
        share_args = generate_physical_xpass.parse_args(["--pc-xpass", "--endpoint-normalization", "share"])
        self.assertEqual(share_args.endpoint_normalization, PC_XPASS_ENDPOINT_NORMALIZATION_SHARE)
        ...
        custom_args = generate_physical_xpass.parse_args(
            [
                ...
                "--max-player-speed",
                "6",
                "--max-player-speed-off",
                "7",
                "--max-player-speed-def",
                "4",
                "--min-speed",
                "2",
                "--radial-gridsize",
        ...
        self.assertEqual(custom_args.min_speed, 2)
        self.assertEqual(custom_args.radial_gridsize, 1.5)
        self.assertEqual(custom_args.lane_power, 12)
        ...
        overall_args = generate_physical_xpass.parse_args(["--pc-xpass", "--max-player-speed", "6"])
        self.assertEqual(overall_args.max_player_speed, 6)
        self.assertIsNone(overall_args.max_player_speed_off)
        self.assertIsNone(overall_args.max_player_speed_def)

        defense_args = generate_physical_xpass.parse_args(["--pc-xpass", "--max-player-speed-def", "4"])
    ...
    def test_generate_physical_xpass_cli_pc_xpass_accepts_split_teammate_ignoring(self) -> None:
        ...
        self.assertTrue(generate_physical_xpass.pc_ignore_teammates_lane_survival_from_args(control_args))
        ...
        for flag in ["--max-player-speed", "--max-player-speed-off", "--max-player-speed-def", "--min-speed", "--radial-gridsize"]:
            with self.subTest(flag=flag):
                ...
                with self.assertRaises(SystemExit):
                    generate_physical_xpass.parse_args([flag, "1"])
        with self.assertRaises(SystemExit):
            generate_physical_xpass.parse_args(["--pc-xpass", "--min-speed", "10", "--max-speed", "8"])
