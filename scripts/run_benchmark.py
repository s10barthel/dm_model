...
def main() -> None:
    ...
    lane_survival_cache_dir = pc_versions.lane_cache_dir("benchmark", args, model_specs)
    ...
    with tqdm(built_states, total=len(built_states), desc="benchmark states") as progress:
        for state_number, (modification_id, game_state_id, state, state_rows, stats) in enumerate(progress, start=1):
            ...
            try:
                ...
                physical_skip_stats = getattr(state, "physical_xpass_skipped_actions", None)
                if physical_skip_stats:
                    physical_xpass_skipped_actions[state_key] = physical_skip_stats
                    state_record["physical_xpass_skipped_actions"] = physical_skip_stats
    ...
    metadata = {
        ...
        "physical_xpass_v5_intent_threshold": physical_lookup_config.get("v5_intent_threshold") if physical_lookup_config.get("weight_version") == "v5" else None,
        "physical_xpass_v5_discount": physical_lookup_config.get("v5_discount") if physical_lookup_config.get("weight_version") == "v5" else None,
        "physical_xpass_pass_intent_model_id": resolved_model_ids["pass_intent"] if physical_lookup_config.get("weight_version") == "v5" else None,
