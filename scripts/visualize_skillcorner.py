...
def _probs_for_component_frame(
    ...
    if component_name in {"outcome_failure", "outcome_success"}:
        ...
        return compute_outcome(
            build_visualization_probs(_row_for_frame(component_tables[f"outcome_scoring_{case}"], frame_id)),
            build_visualization_probs(_row_for_frame(component_tables[f"outcome_conceding_{case}"], frame_id)),
        )
